...
class BlockChain(object):
    ...
    def set_neighbours(self):
        self.neighbours = utils.find_neighbours(
            '127.0.0.1', self.port, NEIGHBOURS_IP_RANGE_NUM[0], NEIGHBOURS_IP_RANGE_NUM[1], BLOCKCHAIN_PORT_RANGE[0], BLOCKCHAIN_PORT_RANGE[1])
    ...
    def resolve_conflicts(self):
        ...
        if longest_chain:
            ...
            logger.info({'action': 'resolve_conflicts', 'status': 'replacad'})
            return True

        logger.info({'action': 'resolve_conflicts', 'status': 'not replacad'})
        return False
